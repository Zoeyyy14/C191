        ...
        window.addEventListener("keydown",function(e){
            ...
            if(e.code=="ArrowUp"){
                ...
                if(multiply<=100 && cameraPosition.z>-500){
                    ...
                    var carSpeed=document.quarySelector("#speed")
                    carSpeed.setAttribute("text", {value:multiply})
                }
            }

            if(e.code=="Space"){
                cameraRig.setAttribute("movement-control",{"speed":0})
                var stopCar=document.querySelector("#control-break")
                stopCar.setAttribute("material", "color", "purple")
            }
        })
        window.addEventListener("keyup",function(e){
            var cameraRig=document.querySelector("#camera-rig")
            var cameraDirection=new THREE.Vector3()
            cameraRig.object3D.gameWorldDirection(cameraDirection)
            var cameraMoveControl=cameraRig.getAttribute("movement-controls")

            if(e.code=="Space"){
                var startCar=document.querySelector("#control-rig")
